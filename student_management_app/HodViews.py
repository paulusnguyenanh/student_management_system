...
def upload_staff(request):
    form = AddStaffForm(request.POST or None, request.FILES or None)
    if form.is_valid():
        form.save()
        form=form


    return render(request,"hod_template/manage_staff_template.html",{"form":form})
...
def staff_pdf(request):
    ...
    for staff in staffs:
        lines.append(staff.admin.username)
        lines.append(staff.admin.first_name)
        lines.append(staff.admin.last_name)
        lines.append("======")
    ...
    return FileResponse(buf, as_attachment=True,filename='staff.pdf')
...
def staff_csv(request):
    ...
    staffs = Staffs.objects.all()
    writer.writerow(['id', 'firstname','lastname','username','mail','adress','gender','pic','lastlogin','date'])

    lines = []
    for staff in staffs:
        writer.writerow([staff.admin.id,staff.admin.first_name,staff.admin.last_name,staff.admin.username,staff.admin.email,staff.address, staff.gender,staff.profile_pic,staff.admin.last_login,staff.admin.date_joined])
    response.writelines(lines)
    return response


def staff_text(request):
    response = HttpResponse(content_type= 'text/plain')
    response['Content-Disposition']= 'attachment; filename=staff.txt'

    staffs = Staffs.objects.all()

    lines = []
    for staff in staffs:
        lines.append(f'{staff.admin.username}\n{staff.admin.first_name}\n{staff.admin.last_name}\n\n\n')
    response.writelines(lines)
    return response
...
def add_subject_save(request):
    if request.method!="POST":
        ...
        try:
            ...
            return HttpResponseRedirect(reverse("add_subject"))
def search_staff(query=None):
	queryset = []
	queries = query.split(" ")
	for q in queries:
		staffs = Staffs.objects.filter(
			Q(admin__first_name__contains=q)

			).distinct()
		for staff in staffs:
			queryset.append(staff)
...
def manage_staff(request):
    ...
    p = Paginator(search_staff(query), 4) # Show 1 contacts mỗi page
    ...
    try:
        ...
        staffs = p.get_page(p.num_pages)


    return render(request,"hod_template/manage_staff_template.html",{"staffs":staffs,"query":query})
...
def edit_staff(request,staff_id):
    ...
    return render(request, "hod_template/edit_staff_template.html",
                  {"form": form, "id": staff_id, "username": staff.admin.username})
def delete_staff(request, staff_id):

    staff = CustomUser.objects.get(id=staff_id)
    staff.delete()
    messages.success(request, "Successfully Delete Staff")
...
def delete_student(request, student_id):
    ...
    student.delete()
    messages.success(request, "Successfully Delete Student")
